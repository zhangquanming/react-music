    ...
    getSingerList(1, `${this.state.typeKey + '_' + this.state.indexKey}`).then(res => {
      if (res && res.code === CODE_SUCCESS) {
        ...
        res.data.list.forEach(data => {
          let singer = new SingerModel.Singer(
            data.Fsinger_id,
            data.Fsinger_mid,
            data.Fsinger_name,
  ...
  render() {
    ...
    let indexs = this.indexs.map(type => (
      <span
        ...
        onClick={() => {
          this.handleIndexClick(type.key);
        }}
      >
        {type.name}
      </span>
          ...
          <div className={style.singerImg}>
            <LazyLoad height={50}>
              <img
                ...
                alt={singer.name}
                onClick={e => {
                  e.currentTarget.src = musicImg;
                }}
              />
            </LazyLoad>
          </div>
          <div className={style.singerName}>{singer.name}</div>
        </div>
      );
    });
    ...
    return (
      <div className="music-singers skin-music-singers">
        ...
        <div className={style.singerList}>
          ...
          >
            <div className={style.singerContainer}>{singers}</div>
          </Scroll>
        </div>
